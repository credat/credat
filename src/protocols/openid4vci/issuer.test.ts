...
describe('CredentialIssuer', () => {
  ...
  describe('createOffer', () => {
    it('creates credential offer with URI', () => {
      ...
      expect(offer.credential_configuration_ids).toEqual(['IdentityCredential'])
      expect(offer.grants['urn:ietf:params:oauth:grant-type:pre-authorized_code']['pre-authorized_code']).toBeTruthy()
      expect(offerUri).toMatch(/^openid-credential-offer:\/\/\?credential_offer=/)
    })
  })
  ...
  describe('exchangePreAuthorizedCode', () => {
    ...
    it('rejects invalid pre-authorized code', () => {
      expect(() => issuer.exchangePreAuthorizedCode('invalid-code')).toThrow(ProtocolError)
    })
    ...
    it('rejects already-used pre-authorized code', () => {
      ...
      expect(() => issuer.exchangePreAuthorizedCode(code)).toThrow(ProtocolError)
    })
    ...
    it('rejects expired pre-authorized code', () => {
      ...
      const code = offer.grants['urn:ietf:params:oauth:grant-type:pre-authorized_code']['pre-authorized_code']
      expect(() => shortLivedIssuer.exchangePreAuthorizedCode(code)).toThrow(ProtocolError)
    })
  })
  ...
  describe('issueCredential', () => {
    it('issues SD-JWT VC credential', async () => {
      ...
      const code = offer.grants['urn:ietf:params:oauth:grant-type:pre-authorized_code']['pre-authorized_code']
      const tokenResponse = issuer.exchangePreAuthorizedCode(code)
      const { response, credential } = await issuer.issueCredential(tokenResponse.access_token, 'vc+sd-jwt')
      ...
      expect(credential.claims).toEqual({ givenName: 'Alice', familyName: 'Smith' })
      expect(credential.id).toMatch(/^urn:uuid:/)

      // Verify the issued credential
      const verifyResult = await verifySdJwtVc(response.credential, config.publicKey)
    ...
    it('issues mDoc credential', async () => {
      ...
      const code = offer.grants['urn:ietf:params:oauth:grant-type:pre-authorized_code']['pre-authorized_code']
      const tokenResponse = issuer.exchangePreAuthorizedCode(code)
      const { response, credential } = await issuer.issueCredential(tokenResponse.access_token, 'mso_mdoc')
    ...
    it('rejects invalid access token', async () => {
      await expect(issuer.issueCredential('invalid-token')).rejects.toThrow(ProtocolError)
    })
    ...
    it('rejects already-used access token', async () => {
      ...
      await expect(issuer.issueCredential(tokenResponse.access_token)).rejects.toThrow(ProtocolError)
    })
    ...
    it('rejects expired access token', async () => {
      ...
      await expect(shortLivedIssuer.issueCredential(tokenResponse.access_token)).rejects.toThrow(ProtocolError)
    })
    ...
    it('rejects unsupported format', async () => {
      ...
      await expect(sdJwtOnlyIssuer.issueCredential(tokenResponse.access_token, 'mso_mdoc')).rejects.toThrow(ProtocolError)
    })
  })
  ...
  describe('getMetadata', () => {
    it('returns issuer metadata with supported formats', () => {
      ...
      expect(metadata.credential_endpoint).toBe('https://issuer.example.com/credential')
      expect(metadata.token_endpoint).toBe('https://issuer.example.com/token')
      expect(metadata.credential_configurations_supported['sd-jwt-vc']).toBeDefined()
