...
import { CredentialIssuer, type CredentialIssuerConfig } from '../openid4vci/issuer'
...
function createVerifierConfig(overrides?: Partial<CredentialVerifierConfig>): CredentialVerifierConfig {
  return {
...
async function issueTestCredential(
  ...
  overrides?: { format?: 'vc+sd-jwt' | 'mso_mdoc'; sd?: string[]; expiresAt?: Date },
) {
  const issuer = new CredentialIssuer(issuerConfig)
  const { offer } = issuer.createOffer({
    credentialType: 'IdentityCredential',
    claims: { givenName: 'Alice', familyName: 'Smith', dateOfBirth: '1990-01-01' },
  ...
  return issuer.issueCredential(tokenResponse.access_token, overrides?.format ?? 'vc+sd-jwt')
}

function buildVpResponse(vpToken: string, format: 'vc+sd-jwt' | 'mso_mdoc', state: string): AuthorizationResponse {
...
describe('CredentialVerifier', () => {
  ...
  describe('createAuthorizationRequest', () => {
    it('creates authorization request with request URI', () => {
      ...
      expect(request.redirect_uri).toBe('https://verifier.example.com/callback')
      ...
      expect(request.presentation_definition.input_descriptors[0]!.constraints.fields).toHaveLength(2)
  ...
  describe('processResponse', () => {
    it('verifies SD-JWT VP token and extracts claims', async () => {
      const { response: credResponse } = await issueTestCredential(issuerConfig)
      ...
      const vpResponse = buildVpResponse(credResponse.credential, 'vc+sd-jwt', request.state)
      const result = await verifier.processResponse(vpResponse, issuerConfig.publicKey)
    ...
    it('rejects untrusted issuer', async () => {
      ...
      const vpResponse = buildVpResponse(credResponse.credential, 'vc+sd-jwt', request.state)
      const result = await trustedVerifier.processResponse(vpResponse, issuerConfig.publicKey)
    ...
    it('rejects unknown state', async () => {
      const vpResponse = buildVpResponse('some-token', 'vc+sd-jwt', 'unknown-state')
      await expect(verifier.processResponse(vpResponse)).rejects.toThrow(ProtocolError)
    })
    ...
    it('rejects already-used state', async () => {
      ...
      await expect(verifier.processResponse(vpResponse, issuerConfig.publicKey)).rejects.toThrow(ProtocolError)
    })
  })
