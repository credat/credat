...
export class CredentialVerifier {
  ...
  constructor(config: CredentialVerifierConfig) {
    this.config = {
      ...config,
      requestExpirySeconds: config.requestExpirySeconds ?? DEFAULT_REQUEST_EXPIRY,
    }
  }
  ...
  createAuthorizationRequest(params: PresentationRequestParams): {
    ...
    const now = new Date()
    const expiresAt = new Date(now.getTime() + this.config.requestExpirySeconds! * 1000)
  ...
  static buildPresentationDefinition(params: PresentationRequestParams): PresentationDefinition {
  ...
  private async verifySdJwtPresentation(
    ...
    const pubKey = issuerPublicKey ?? (await this.resolveIssuerKeyFromSdJwt(vpToken))
  ...
  private async resolveIssuerKeyFromSdJwt(sdJwt: string): Promise<Uint8Array | null> {
    try {
      ...
      if (!resolution.didDocument?.verificationMethod?.[0]?.publicKeyJwk) return null

      return jwkToPublicKey(resolution.didDocument.verificationMethod[0].publicKeyJwk)
    } catch {
      return null
    }
  }
...
function buildPresentationDefinition(params: PresentationRequestParams): PresentationDefinition {
