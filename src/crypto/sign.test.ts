...
describe("sign and verify", () => {
	it("signs and verifies with ES256", () => {
		...
		expect(verifySignature(payload, signature, kp.publicKey, "ES256")).toBe(
			true,
		);
	});
	...
	it("signs and verifies with EdDSA", () => {
		...
		expect(verifySignature(payload, signature, kp.publicKey, "EdDSA")).toBe(
			true,
		);
	});
	...
	it("rejects tampered payload", () => {
		...
		expect(verifySignature(tampered, signature, kp.publicKey, "ES256")).toBe(
			false,
		);
	});
	...
	it("rejects wrong public key", () => {
		...
		expect(verifySignature(payload, signature, kp2.publicKey, "ES256")).toBe(
			false,
		);
	});
});
