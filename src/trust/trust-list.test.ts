...
import { EuTrustList, CustomTrustList, isIssuerTrusted } from './trust-list'

describe('EuTrustList', () => {
  const trustList = new EuTrustList()

  it('recognizes known EU issuer as trusted', async () => {
    const result = await trustList.isIssuerTrusted('did:web:pid-issuer.bundesdruckerei.de')
    expect(result).toBe(true)
  })

  it('rejects unknown issuer as untrusted', async () => {
    const result = await trustList.isIssuerTrusted('did:web:unknown-issuer.example.com')
...
describe('CustomTrustList', () => {
  it('trusts issuers in the custom list', async () => {
    const trustList = new CustomTrustList('my-org', ['did:key:zIssuer1', 'did:key:zIssuer2'])
...
describe('isIssuerTrusted helper', () => {
  it('uses EU trust list when specified', async () => {
    const result = await isIssuerTrusted('did:web:pid-issuer.bundesdruckerei.de', 'eu')
  ...
  it('uses custom trust list with provided issuers', async () => {
    const result = await isIssuerTrusted('did:key:zMyIssuer', 'custom', ['did:key:zMyIssuer'])
    expect(result.issuerTrusted).toBe(true)
