...
describe("ErrorCodes", () => {
	...
	it("contains all delegation error codes", () => {
		...
		expect(ErrorCodes.DELEGATION_SIGNATURE_INVALID).toBe(
			"DELEGATION_SIGNATURE_INVALID",
		);
		expect(ErrorCodes.DELEGATION_SCOPE_INVALID).toBe(
	...
	it("contains all handshake error codes", () => {
		...
		expect(ErrorCodes.HANDSHAKE_VERIFICATION_FAILED).toBe(
